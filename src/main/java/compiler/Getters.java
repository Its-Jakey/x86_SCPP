...
public class Getters {
    ...
    public static void addConstant(String name, int value) {
        if (hasConstant(name))
    ...
    public static Variable getVariable(Namespace namespace, Function function, String name) {
        ...
        if (function == null || !(function.arguments.containsKey(name) || function.localVariables.containsKey(name))) {
            ...
            return new Variable(getConstant(name));
        }
        if (function.arguments.containsKey(name)) {
            Variable ret = new Variable(function.arguments.get(name));
            ret.isPublic = false;
            return ret;
        }
        Variable ret = new Variable(function.localVariables.get(name));
        ret.isPublic = false;
        return ret;
    }
